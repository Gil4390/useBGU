...
\label{sec:org3abc47a}
\chapter{Introduction}
\label{sec:orgcb60dbc}
...
\textbf{MLM-USE} (Part \ref{sec:orga1bec65}) is where multi-level modeling
...
\textbf{CatUSE} (Part \ref{sec:orge58ddb4}, also called CatMLM in the source) is a
...
Chapter \ref{sec:org63541d4}) can only be written in full
...
Chapter \ref{sec:org06db68c}.
\end{description}

\section{How to Read This Manual}
\label{sec:org5fa921c}
Part \ref{sec:org3abc47a} gets the system installed, built, and
running, and ends with a short guided walkthrough (Chapter
\ref{sec:orgd9cc398}). Part \ref{sec:org2c3ff8a} documents plain
USE in full. Parts \ref{sec:orga1bec65} and \ref{sec:orge58ddb4} cover MLM-USE
and CatUSE respectively, each in the same shape: concepts, syntax,
worked examples. Part \ref{sec:org745982f} is for anyone modifying this
fork's own implementation, not just writing models in it. Part
\ref{sec:orgc5ae712} covers where USE and this fork came from. The
...
\ref{sec:org46d74f9} for what that distinction means and why it
...
\ref{sec:org3abc47a} can be skimmed and Part \ref{sec:org2c3ff8a}
skipped entirely.
\end{manualnote}
\chapter{Installing and Building}
\label{sec:orgf97e73e}
\section{Prerequisites}
\label{sec:org3029462}
\index{JDK}
\index{Maven}
...
\ref{sec:orgfe2e8a9} runs directly.
...
compiler itself (Part \ref{sec:org745982f}) than at someone writing
models.

\section{The Shell Runs Underneath the GUI, Too}
\label{sec:org7afd954}
...
\ref{sec:org66d4965} covers the shell's own command
language in full.

\section{Opening a Specification: Four Entry Points}
\label{sec:orgf62dfcf}
...
Action & Parses\\[0pt]
\hline
Open specification\ldots{} & A plain, single \texttt{model} (Part \ref{sec:org2c3ff8a})\\[0pt]
Open multi specification\ldots{} & Several ordinary models glued by \texttt{inter-*} sections, no levels\\[0pt]
Open multi level specification\ldots{} & An \texttt{MLM} file: levels, clabjects, mediators (Part \ref{sec:orga1bec65})\\[0pt]
Open CatUSE specification\ldots{} & A CatUSE/CatMLM file (Part \ref{sec:orge58ddb4})\\[0pt]
\end{tabular}
\end{center}
...
\label{sec:org32606a9}
The multi-level-modeling extension, as a \textbf{delta} on top of the USE
grammar in Appendix \ref{sec:orgd28d585}: only rules that are new or
...
\label{sec:org2df123a}
The CatMLM (``CatUSE'') front-end syntax sugar, as a \textbf{delta} on top of
MLM-USE (Appendix \ref{sec:org32606a9}): CatMLM desugars into
...
\label{sec:org46d74f9}
This chapter is a placeholder.
\chapter{Shell Command Reference}
\label{sec:orgbffd437}
This chapter is a placeholder.
